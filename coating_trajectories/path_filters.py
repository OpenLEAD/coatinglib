...
def _mh12_filter(turbine, trajectories):
    ...
    for trajectory in trajectories:
        ...
        boo = distance_robot_squared(trajectory[:,0:3]) < _working_radius_squared
        indices = np.nonzero(boo[1:] != boo[:-1])[0] + 1
        split_trajectory = np.split(trajectory, indices)
        filtered_trajectory_list = split_trajectory[0::2] if boo[0] else split_trajectory[1::2]
        if boo[0] & boo[-1]:
            filtered_trajectory_list[0] = np.concatenate((filtered_trajectory_list[-1],filtered_trajectory_list[0]))
