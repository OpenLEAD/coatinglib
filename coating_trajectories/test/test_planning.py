...
from numpy import concatenate, array, random, arange, dot, sqrt, sin, cos, ones, sum, zeros
from math import pi
...
class TestPlanning(TestCase):
    ...
    def test_compute_robot_joints(self):
        ...
        ny = (2*cos(theta)+cos(2*theta)-2*cos(4*theta))/(-2*sin(theta)+sin(2*theta)+2*sin(4*theta))
        nx = ones(len(theta))
        nz = zeros(len(theta))
        ...
        trajectory = [[positions[i][0], positions[i][1], positions[i][2],
                       normal_vectors[i][0], normal_vectors[i][1], normal_vectors[i][2]] for i in range(0,len(positions))]
        joint_solutions = planning.compute_robot_joints(turb, trajectory)
