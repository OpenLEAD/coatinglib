...
for i in range(0,len(Torques)):
    ...
    for j in range(0,len(Torques[i])):
        if (Torques[i][j]<=maxTorques).all()==True:
            ...
            torquetrue.append(alltriopoints[i][0][0])
            io = 0
            break
    if io:
...
Axes3D.scatter(ax, nx, ny, nz,s=50,c='r')
Axes3D.scatter(ax, x, y, z,c='g')
ax.axis("off")

savez_compressed('torquefail0_HD.npz', array=torquefail)
savez_compressed('torquetrue0_HD.npz', array=torquetrue)
